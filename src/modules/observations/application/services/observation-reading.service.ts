...
@Injectable()
export class ObservationReadingService
  implements InterfaceObservationReadingUseCase
{
  constructor(
  ...
  async getObservations(): Promise<ObservationDetailsResponse[]> {
    try {
      const models = await this.observationRepository.getObservations();
      return models.map(
        ObservationReadingMapper.fromObservationDetailsModelToResponse,
      );
    } catch (error) {
      throw error;
    }
  ...
  async createObservationReading(
    ...
    try {
      ...
      const observationModel: ObservationReadingModel =
      ...
      return ObservationReadingMapper.fromObservationReadingModelToResponse(
        createdObservation,
      );
    } catch (error) {
      throw error;
  ...
  async getObservationsByReadingId(
    ...
    try {
      const models =
        await this.observationRepository.getObservationsByReadingId(readingId);
      return models.map(
        ObservationReadingMapper.fromObservationReadingModelToResponse,
      );
    } catch (error) {
      throw error;
    }
  ...
  async getObservationDetailsByCadastralKey(
    ...
    try {
      ...
      const resultModels =
        await this.observationRepository.getObservationDetailsByCadastralKey(
          cadastralKey,
        );
