  ...
  implements InterfaceObservationReadingRepository
{
  ...
  async getObservations(): Promise<ObservationDetailsModel[]> {
    try {
      const query: string = `
        SELECT
  ...
  async getObservationDetailsByCadastralKey(
    ...
    try {
      ...
      const result = await this.postgresqlService.query<any>(query, params);
      return result.map(ObservationReadingSQLAdapter.toObservationDetailsModel);
    } catch (error) {
      throw error;
    }
  ...
  async createObservationReading(
    ...
    try {
      ...
      return ObservationReadingSQLAdapter.toObservationReadingModel(
        resultSelect[0],
      );
    } catch (error) {
      throw error;
